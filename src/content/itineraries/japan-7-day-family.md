---
title: "Japan in 7 days: Tokyo and Kyoto"
country: "Japan"
days: 7
budgetPerPerson: "TBC"
bestFor: "First-time visitors"
heroImage: "/images/japan.jpeg"
heroWide: "/images/japan-wide.jpeg"
summary: "Four nights in Tokyo and three in Kyoto, joined by one Shinkansen ride — two bases, one train, no repacking in between."
publishDate: 2026-09-14
draft: false
---

<!--
  STUB — draft: true, so this is not on the site or in the sitemap yet.

  Before publishing, confirm and fill in:
  - Entry rules: Japan is visa-free for some nationalities and not others,
    and has been expanding an eVisa route. Cover it in the body rather than
    the frontmatter, and date it.
  - budgetPerPerson: Japan is the most expensive destination on this site so
    far — price it honestly rather than anchoring to the Georgia number.
  - Whether a rail pass beats point-to-point tickets for this route; it often
    does not for a single Tokyo-Kyoto leg.
-->

Japan rewards staying put. Two bases over a week means one train journey
rather than five, and the Shinkansen between them is short enough that
the travel day still has an afternoon in it.

## Day 1 — Tokyo: land, settle, and stay close to the station

Pick a hotel near a JR loop station so the rest of the week is simple.
Keep the first evening light — a neighbourhood walk and an early dinner
beats trying to see anything after a long flight.

<a class="book-btn" href="#PASTE-TRAVELPAYOUTS-HOTEL-LINK">Book a hotel near Tokyo Station</a>
